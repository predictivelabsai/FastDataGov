...
service = next(
    (name for name, spec in catalog().items() if spec.get("local_dir") == ROOT.name),
    None,
)
if not service:
...
if len(sys.argv) < 2:
    raise SystemExit(
        "usage: coolify.py validate|doctor|status|provision|env|deploy [options]"
    )
command, *options = sys.argv[1:]
sys.argv = [
    sys.argv[0],
    command,
    *([] if command == "validate" else [service]),
    *options,
]
main()
