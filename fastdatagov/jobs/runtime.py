...
def _adapter_for_connection(connection_id: int):
    row = fetch_one(
        ...
        FROM fast_datagov.connections c
        JOIN fast_datagov.platforms p ON p.id=c.platform_id
        WHERE c.id=%s
        """,
        (connection_id,),
    )
...
def sync_connection(connection_id: int) -> dict:
    ...
    with connect() as connection, connection.cursor() as cursor:
        cursor.execute("SELECT sync_cursor FROM fast_datagov.connections WHERE id=%s", (connection_id,))
        sync_cursor = cursor.fetchone()[0] or {}
        cursor.execute(
            "INSERT INTO fast_datagov.sync_runs (connection_id, status, cursor_before) VALUES (%s, 'running', %s) RETURNING id",
        ...
        with adapter:
            ...
            with connect() as connection, connection.cursor() as cursor:
                for record in asset_page.records:
                    cursor.execute(
                        """
                        INSERT INTO fast_datagov.assets
                ...
                for field in adapter.discover_fields(external_ids):
                    cursor.execute("SELECT id FROM fast_datagov.assets WHERE connection_id=%s AND external_id=%s", (connection_id, field.asset_external_id))
                    asset_row = cursor.fetchone()
                    if not asset_row:
                        continue
                    cursor.execute(
                        """
                        INSERT INTO fast_datagov.asset_fields
                ...
                cursor.execute("SELECT external_id FROM fast_datagov.assets WHERE connection_id=%s AND deleted_at IS NULL",(connection_id,))
                governed_external_ids=[row[0] for row in cursor.fetchall()]
                cursor.execute("DELETE FROM fast_datagov.asset_tags atag USING fast_datagov.assets a WHERE a.id=atag.asset_id AND a.connection_id=%s AND atag.source<>'fastdatagov'",(connection_id,))
                tag_records = adapter.discover_tags(governed_external_ids)
                for tag in tag_records:
                    cursor.execute("SELECT id FROM fast_datagov.assets WHERE connection_id=%s AND external_id=%s", (connection_id, tag.asset_external_id))
                    asset_row = cursor.fetchone()
                    if not asset_row:
                        continue
                    cursor.execute("INSERT INTO fast_datagov.tags (key, label, category) VALUES (%s,%s,'platform') ON CONFLICT (key) DO UPDATE SET label=excluded.label RETURNING id", (tag.key, tag.key))
                    tag_id = cursor.fetchone()[0]
                    cursor.execute("INSERT INTO fast_datagov.asset_tags (asset_id, tag_id, source, tag_value) VALUES (%s,%s,%s,%s) ON CONFLICT (asset_id, tag_id) DO UPDATE SET source=excluded.source,tag_value=excluded.tag_value", (asset_row[0], tag_id, tag.source,tag.value))
                    counts["tags"] += 1
                cursor.execute("DELETE FROM fast_datagov.asset_visibility av USING fast_datagov.assets a WHERE a.id=av.asset_id AND a.connection_id=%s AND av.source<>'fastdatagov'",(connection_id,))
                for grant in adapter.discover_grants(governed_external_ids):
                    cursor.execute("SELECT id FROM fast_datagov.assets WHERE connection_id=%s AND external_id=%s", (connection_id, grant.asset_external_id))
                    asset_row = cursor.fetchone()
                    if not asset_row:
                        continue
                    cursor.execute("INSERT INTO fast_datagov.asset_visibility (asset_id,principal_type,principal_key,privilege,source) VALUES (%s,%s,%s,%s,%s) ON CONFLICT (asset_id,principal_type,principal_key,privilege) DO UPDATE SET source=excluded.source", (asset_row[0], grant.principal_type, grant.principal_key, grant.privilege, grant.source))
                    counts["grants"] += 1
                connection.commit()
            ...
            with connect() as connection, connection.cursor() as cursor:
                for edge in lineage_page.records:
                    cursor.execute("SELECT id FROM fast_datagov.assets WHERE connection_id=%s AND external_id=%s", (connection_id, edge.source_external_id)); source = cursor.fetchone()
                    cursor.execute("SELECT id FROM fast_datagov.assets WHERE connection_id=%s AND external_id=%s", (connection_id, edge.target_external_id)); target = cursor.fetchone()
                    if not source or not target:
                        continue
                    source_field_id=target_field_id=None
                    if edge.source_field:
                        cursor.execute("SELECT id FROM fast_datagov.asset_fields WHERE asset_id=%s AND lower(name)=lower(%s)",(source[0],edge.source_field)); field=cursor.fetchone(); source_field_id=field[0] if field else None
                    if edge.target_field:
                        cursor.execute("SELECT id FROM fast_datagov.asset_fields WHERE asset_id=%s AND lower(name)=lower(%s)",(target[0],edge.target_field)); field=cursor.fetchone(); target_field_id=field[0] if field else None
                    cursor.execute(
                        """
                        INSERT INTO fast_datagov.lineage_edges
                ...
                for usage in usage_page.records:
                    cursor.execute("SELECT id FROM fast_datagov.assets WHERE connection_id=%s AND external_id=%s", (connection_id, usage.asset_external_id)); asset_row = cursor.fetchone()
                    if not asset_row:
                        continue
                    cursor.execute(
                        """
                        INSERT INTO fast_datagov.usage_rollups (asset_id, usage_date, query_count, distinct_users)
                        VALUES (%s,%s,%s,%s)
                ...
                cursor.execute("UPDATE fast_datagov.connections SET sync_cursor=%s, status='healthy', last_sync_at=now(), last_error=NULL, updated_at=now() WHERE id=%s", (Jsonb(combined_cursor), connection_id))
                cursor.execute("UPDATE fast_datagov.sync_runs SET status='succeeded', cursor_after=%s, assets_seen=%s, fields_seen=%s, lineage_edges_seen=%s, completed_at=now() WHERE id=%s", (Jsonb(combined_cursor), counts["assets"], counts["fields"], counts["lineage"], sync_run_id))
                cursor.execute("INSERT INTO fast_datagov.audit_events (actor_subject, action, entity_type, entity_key, after_data) VALUES (%s,'adapter.sync_completed','connection',%s,%s)", (f"adapter:{adapter.key}", str(connection_id), Jsonb(counts)))
                connection.commit()
        return counts
    except Exception as exc:
        with connect() as connection, connection.cursor() as cursor:
            cursor.execute("UPDATE fast_datagov.connections SET status='error', last_error=%s, updated_at=now() WHERE id=%s", (str(exc)[:2000], connection_id))
            cursor.execute("UPDATE fast_datagov.sync_runs SET status='failed', error_summary=%s, completed_at=now() WHERE id=%s", (str(exc)[:2000], sync_run_id))
            connection.commit()
        raise
...
def run_quality(rule_id: int) -> dict:
    ...
    with connect() as connection, connection.cursor() as cursor:
        cursor.execute("INSERT INTO fast_datagov.quality_runs (rule_id, status, score, observed_value, rows_evaluated, message, completed_at, evidence) VALUES (%s,%s,%s,%s,%s,%s,now(),%s)", (rule_id, result.status, result.score, result.observed_value, result.rows_evaluated, result.message, Jsonb(result.evidence)))
        cursor.execute("""UPDATE fast_datagov.assets SET quality_score=(SELECT avg(latest.score) FROM fast_datagov.quality_rules qr2 JOIN LATERAL (SELECT score FROM fast_datagov.quality_runs qrun WHERE qrun.rule_id=qr2.id AND qrun.completed_at IS NOT NULL ORDER BY qrun.started_at DESC LIMIT 1) latest ON true WHERE qr2.asset_id=%s), updated_at=now() WHERE id=%s""", (row["asset_id"], row["asset_id"]))
        ...
        if result.status == "failed":
            ...
            cursor.execute("""INSERT INTO fast_datagov.work_items (kind,asset_id,title,description,priority,assignee_email,payload)
                              SELECT 'quality',%s,%s,%s,%s,%s,%s WHERE NOT EXISTS
                              (SELECT 1 FROM fast_datagov.work_items WHERE kind='quality' AND status NOT IN ('resolved','rejected') AND payload->>'rule_id'=%s)""", (row["asset_id"], f"Resolve: {row['name']}", result.message, row["severity"], assignee, Jsonb({"rule_id": rule_id}),str(rule_id)))
            cursor.execute("""INSERT INTO fast_datagov.notification_outbox (channel_id,event_type,recipient,subject,body,payload)
                              SELECT id,'quality.failed',%s,%s,%s,%s FROM fast_datagov.notification_channels
        ...
        cursor.execute("INSERT INTO fast_datagov.audit_events (actor_subject,action,entity_type,entity_key,after_data) VALUES ('worker','quality.run_completed','quality_rule',%s,%s)",(str(rule_id),Jsonb({"status":result.status,"score":result.score,"rows_evaluated":result.rows_evaluated})))
        connection.commit()
...
def check_connection_health(connection_id: int) -> dict:
    ...
    with connect() as connection, connection.cursor() as cursor:
        cursor.execute("INSERT INTO fast_datagov.connection_health_checks (connection_id,status,message,latency_ms) VALUES (%s,%s,%s,%s)",(connection_id,status,message,latency))
        cursor.execute("UPDATE fast_datagov.connections SET status=%s,last_error=%s,updated_at=now() WHERE id=%s",(status,None if status in {"healthy","ready"} else message,connection_id)); connection.commit()
