...
def schedule_due() -> dict[str, int]:
    ...
    with connect() as connection, connection.cursor() as cursor:
        cursor.execute("UPDATE fast_datagov.jobs SET status='queued',locked_by=NULL,locked_at=NULL,run_after=now(),last_error=coalesce(last_error,'')||' [worker lease expired]' WHERE status='running' AND locked_at<now()-make_interval(secs=>%s)",(settings().job_lease_seconds,))
        counts["recovered"]=cursor.rowcount
        cursor.execute("UPDATE fast_datagov.sync_runs SET status='failed',error_summary='Worker lease expired',completed_at=now() WHERE status='running' AND started_at<now()-make_interval(secs=>%s)",(settings().job_lease_seconds,))
        cursor.execute(
            """
            INSERT INTO fast_datagov.jobs (kind, payload)
            ...
            WHERE coalesce(c.last_sync_at, 'epoch'::timestamptz) < now() - make_interval(mins => %s)
              AND NOT EXISTS (SELECT 1 FROM fast_datagov.jobs j WHERE j.kind='adapter.sync'
        ...
        cursor.execute(
            ...
            WHERE qr.enabled=true AND qr.schedule<>'manual'
              ...
              AND NOT EXISTS (SELECT 1 FROM fast_datagov.jobs j WHERE j.kind='quality.run'
                              AND j.status IN ('queued','running')
        ...
        cursor.execute("""UPDATE fast_datagov.quality_rules SET next_run_at=CASE schedule WHEN 'hourly' THEN now()+interval '1 hour' WHEN 'daily' THEN now()+interval '1 day' WHEN 'weekly' THEN now()+interval '7 days' ELSE NULL END WHERE enabled AND schedule<>'manual' AND coalesce(next_run_at,'epoch'::timestamptz)<=now()""")
        cursor.execute(
            """
            INSERT INTO fast_datagov.jobs (kind,payload)
            SELECT 'notification.send',jsonb_build_object('notification_id',o.id)
            FROM fast_datagov.notification_outbox o
            WHERE o.status='queued' AND o.available_at<=now()
              AND NOT EXISTS (SELECT 1 FROM fast_datagov.jobs j WHERE j.kind='notification.send'
                              AND j.status IN ('queued','running')
        ...
        cursor.execute(
            """INSERT INTO fast_datagov.work_items (kind,asset_id,title,description,priority,assignee_email,due_at,payload)
               ...
               FROM fast_datagov.accountability_assignments aa
               WHERE aa.attestation_expires_at BETWEEN now() AND now()+interval '30 days'
                 AND NOT EXISTS (SELECT 1 FROM fast_datagov.work_items w WHERE w.kind='attestation'
        ...
        cursor.execute(
            """INSERT INTO fast_datagov.work_items (kind,asset_id,title,description,priority,assignee_email,due_at,payload)
               ...
               FROM fast_datagov.certifications c JOIN fast_datagov.assets a ON a.id=c.asset_id
               WHERE c.expires_at BETWEEN now() AND now()+interval '30 days'
                 AND c.id=(SELECT max(c2.id) FROM fast_datagov.certifications c2 WHERE c2.asset_id=c.asset_id)
                 AND NOT EXISTS (SELECT 1 FROM fast_datagov.work_items w WHERE w.kind='certification'
                                 AND w.status NOT IN ('resolved','rejected') AND w.payload->>'certification_id'=c.id::text)"""
        )
        counts["renewals"] += cursor.rowcount
        cursor.execute(
            """INSERT INTO fast_datagov.work_items (kind,title,description,priority,assignee_email,due_at,payload)
               ...
               FROM fast_datagov.product_certifications pc JOIN fast_datagov.data_products p ON p.id=pc.product_id
               WHERE pc.expires_at BETWEEN now() AND now()+interval '30 days'
                 AND pc.id=(SELECT max(pc2.id) FROM fast_datagov.product_certifications pc2 WHERE pc2.product_id=pc.product_id)
                 AND NOT EXISTS (SELECT 1 FROM fast_datagov.work_items w WHERE w.kind='certification'
