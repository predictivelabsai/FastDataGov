...
CREATE TABLE fastdatagov.data_products (
...
CREATE TABLE fastdatagov.product_assets (
...
CREATE TABLE fastdatagov.accountability_assignments (
    id BIGSERIAL PRIMARY KEY,
...
CREATE INDEX accountability_scope_idx ON fastdatagov.accountability_assignments(scope_type, scope_id, responsibility);

CREATE TABLE fastdatagov.asset_revisions (
    id BIGSERIAL PRIMARY KEY,
    asset_id BIGINT NOT NULL REFERENCES fastdatagov.assets(id) ON DELETE CASCADE,
...
CREATE TABLE fastdatagov.glossary_term_revisions (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE fastdatagov.quality_rule_revisions (
    id BIGSERIAL PRIMARY KEY,
...
ALTER TABLE fastdatagov.quality_rules ADD COLUMN next_run_at TIMESTAMPTZ;
ALTER TABLE fastdatagov.quality_rules ADD COLUMN execution_engine TEXT NOT NULL DEFAULT 'platform_sql';
ALTER TABLE fastdatagov.lineage_edges ADD COLUMN created_by TEXT;
ALTER TABLE fastdatagov.lineage_edges ADD COLUMN reviewed_at TIMESTAMPTZ;
ALTER TABLE fastdatagov.lineage_edges ADD COLUMN reviewed_by TEXT;
ALTER TABLE fastdatagov.certifications ADD COLUMN renewal_of_id BIGINT REFERENCES fastdatagov.certifications(id) ON DELETE SET NULL;
ALTER TABLE fastdatagov.certifications ADD COLUMN scope_type TEXT NOT NULL DEFAULT 'asset';

CREATE TABLE fastdatagov.work_item_comments (
    id BIGSERIAL PRIMARY KEY,
    work_item_id BIGINT NOT NULL REFERENCES fastdatagov.work_items(id) ON DELETE CASCADE,
    author_email TEXT NOT NULL,
    body TEXT NOT NULL CHECK (length(trim(body)) > 0),
    created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE TABLE fastdatagov.work_item_events (
    id BIGSERIAL PRIMARY KEY,
    work_item_id BIGINT NOT NULL REFERENCES fastdatagov.work_items(id) ON DELETE CASCADE,
...
CREATE INDEX work_item_events_item_idx ON fastdatagov.work_item_events(work_item_id, created_at);

CREATE TABLE fastdatagov.workflow_definitions (
    id BIGSERIAL PRIMARY KEY,
...
INSERT INTO fastdatagov.workflow_definitions (kind, display_name, default_assignee_role, due_days, approval_role)
VALUES
...
CREATE TABLE fastdatagov.notification_channels (
    id BIGSERIAL PRIMARY KEY,
    key TEXT NOT NULL UNIQUE,
...
CREATE TABLE fastdatagov.notification_outbox (
    id BIGSERIAL PRIMARY KEY,
    channel_id BIGINT REFERENCES fastdatagov.notification_channels(id) ON DELETE SET NULL,
    event_type TEXT NOT NULL,
    recipient TEXT,
...
CREATE INDEX notification_outbox_claim_idx ON fastdatagov.notification_outbox(status, available_at, id);

CREATE TABLE fastdatagov.connection_health_checks (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE fastdatagov.pilot_metrics (
    id BIGSERIAL PRIMARY KEY,
    key TEXT NOT NULL UNIQUE,
    label TEXT NOT NULL,
    unit TEXT NOT NULL,
...
CREATE TABLE fastdatagov.application_settings (
...
INSERT INTO fastdatagov.pilot_metrics (key, label, unit, target, notes)
VALUES
