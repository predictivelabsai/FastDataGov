...
def deliver(notification_id: int) -> None:
    row=fetch_one("SELECT o.*,c.channel_type,c.endpoint_ref FROM fast_datagov.notification_outbox o JOIN fast_datagov.notification_channels c ON c.id=o.channel_id WHERE o.id=%s AND o.status IN ('queued','sending') AND c.enabled",(notification_id,))
    ...
    with connect() as connection, connection.cursor() as cursor:
        cursor.execute("UPDATE fast_datagov.notification_outbox SET status='sent',sent_at=now(),attempts=attempts+1,last_error=NULL WHERE id=%s",(notification_id,)); connection.commit()


def fail(notification_id: int, error: Exception, terminal: bool = False) -> None:
    with connect() as connection, connection.cursor() as cursor:
        cursor.execute("UPDATE fast_datagov.notification_outbox SET status=%s,attempts=attempts+1,last_error=%s,available_at=now()+interval '1 minute' WHERE id=%s",("failed" if terminal else "queued",str(error)[:2000],notification_id)); connection.commit()
